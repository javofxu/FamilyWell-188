...
public class HekrUserAction {
                ...
                public void getStatusSuccess(List<DeviceStatusBean> deviceStatusBeanLists) {
                    ...
                    if (deviceStatusBeanLists != null && !deviceStatusBeanLists.isEmpty()) {
                        ...
                        if (deviceStatusBean.isForceBind() || !deviceStatusBean.isBindToUser()) {
                            String name = "";
                            try {
                                if(TextUtils.isEmpty(deviceStatusBean.getCidName())){
                                    name = mContext.get().getResources().getString(R.string.gateway);
                                }else {
                                    name = (deviceStatusBean.getCidName().substring(deviceStatusBean.getCidName().indexOf("/") + 1));
                                }
                            }catch (Exception e){
                                name = mContext.get().getResources().getString(R.string.gateway);
                            }
    ...
    public void unPushAllTagBind(final HekrUser.UnPushTagBindListener unPushTagBindListener) {
        CharSequence url = TextUtils.concat(Constants.UrlUtil.BASE_USER_URL, SiterConstantsUtil.UrlUtil.UNPUSH_ALL_ALIAS_BIND);

        deleteHekrData(url, new GetHekrDataListener() {
            @Override
            public void getSuccess(Object object) {
                Log.i(TAG, "推送解绑所有标签接口调用成功");
                unPushTagBindListener.unPushTagBindSuccess();
            }

            @Override
            public void getFail(int errorCode) {
                Log.i(TAG, "推送解绑所有标签接口调用失败");
                unPushTagBindListener.unPushTagBindFail(errorCode);
            }
        });
