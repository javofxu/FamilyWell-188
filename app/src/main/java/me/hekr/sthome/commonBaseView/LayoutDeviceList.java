...
//        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
    ...
    public void updateDeviceList() {
        ...
        List<ApplicationInfo> list = ED.findAllEqByNoPack(ConnectionPojo.getInstance().deviceTid);
        list = updateDeviceList(list);
        LogUtil.d(TAG, "[RYAN] updateDeviceList > list size = " + list.size());

        adapter.update(list);
    }

    private List<ApplicationInfo> updateDeviceList(List<ApplicationInfo> list) {
        List<ApplicationInfo> list2 = new ArrayList<>();
...
//            holder.imageView.setImageResource(mImage[16]);
        ...
        PackDAO PDO = new PackDAO(activity);
    ...
    @Override
    public void onEquipmentClicked(EquipmentBean device) {
        onAppClicked(device);
    }
