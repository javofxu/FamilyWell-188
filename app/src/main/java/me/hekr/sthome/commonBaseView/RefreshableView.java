...
/**
 * RefreshableView:下拉刷新的自定义控件。
 * 作者：Henry on 2017/3/15 9:18
...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	public void setRrefresh(){
		currentStatus = STATUS_REFRESHING;
		new RefreshingTask().execute(1);
	}
	...
	private void setIsAbleToPull(MotionEvent event) {
		...
		if(listView instanceof LinearLayout) {
			View firstChild = ((LinearLayout) listView).getChildAt(0);
			if (firstChild != null) {
				if (!ableToPull) {
					yDown = event.getRawY();
				}
