...
public class CoderUtils {
    ...
    */
    public static String getStringFromAscii(String input){
        ...
        try {
            ...
            String name  = new String(a,encode);
            if(name.indexOf("$")==-1){
                return "";
            }
            int index = -1;
    ...
    */
    ...
    public static String getAscii(String input){
        ...
        try {
            String encode = CacheUtil.getString(SiterSDK.SETTINGS_CONFIG_ENCODE,"GBK");
            countf = 15 - input.getBytes(encode).length;
        ...
        try {
            String encode = CacheUtil.getString(SiterSDK.SETTINGS_CONFIG_ENCODE,"GBK");
            nameBt = newname.getBytes(encode);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    public static String getEncrypt(String input){
        ...
        try {
            String encode = CacheUtil.getString(SiterSDK.SETTINGS_CONFIG_ENCODE,"GBK");
        byte[] nameBt = new byte[input.getBytes(encode).length];
        try {
            nameBt = input.getBytes(encode);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            ...
            for(int i = 0 ; i <Integer.parseInt(slist.get(slist.size()-1).getSid())+1 ; i++) {//for here come with "0" , used slist.size()
                ...
                if (sid.contains(Integer.toString(i))) {
                    ...
                    for (int j = 0; j < sysSceneList.size(); j++) {
                        if(Integer.parseInt(sysSceneList.get(j).getMid())<=128){
                            scene++;
                            length++;
                            String singleCode = "";
                            ...
                            if("129".equals(sysSceneList.get(i).getMid())){
                                scene_default = (byte)(scene_default|0x81);
                            }else if("130".equals(sysSceneList.get(i).getMid())){
                                scene_default = (byte)(scene_default|0x82);
                            }else if("131".equals(sysSceneList.get(i).getMid())){
