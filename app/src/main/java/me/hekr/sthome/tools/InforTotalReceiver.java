...
public abstract class InforTotalReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)){
            ...
            ViewWindow.showView(context,msg,R.color.red);
        }
        ...
        try {
                ...
                if("devSend".equals(json.getString("action"))){
                    ...
                    }else if(data.getInt("cmdId")==SendCommand.UPLOAD_DATASWITCH){
                           String devType = data.getString("device_name");

                           if(NameSolve.DATA_SWITCH.equals(NameSolve.getEqType(devType))){

                               String content = data.getString("deviceId_string");
                               try {
                                   if(content.length()>2){
                                       int count = Integer.parseInt(content.substring(0,2),16);
                                       if(content.length()==(12*count+2)){
                                           int eqid = data.getInt("numbers");
                                           int len = (content.length()-2)/12;
                                           List<DataSwitchSubBean> list = new ArrayList<>();
                                           for(int i=0;i<len;i++){
                                               int address = Integer.parseInt(content.substring(2+i*12+2,2+i*12+8),16);
                                               int status = Integer.parseInt( content.substring(2+i*12+8,2+i*12+10),16);
                                               DataSwitchSubBean dataSwitchSubBean = new DataSwitchSubBean();
                                               dataSwitchSubBean.setEqid(String.valueOf(eqid));
                                               dataSwitchSubBean.setDeviceid(devTids);
                                               dataSwitchSubBean.setSubid(address);
                                               dataSwitchSubBean.setStatus(status);
                                               list.add(dataSwitchSubBean);
                                           }
                                          resolveData.insertDataSwitchList(list);
                                           DataSwitchRefreshEvent dataSwitchRefreshEvent = new DataSwitchRefreshEvent();
                                           dataSwitchRefreshEvent.setDeviceid(devTids);
                                           dataSwitchRefreshEvent.setEqid(String.valueOf(eqid));
                                           dataSwitchRefreshEvent.setList(list);
