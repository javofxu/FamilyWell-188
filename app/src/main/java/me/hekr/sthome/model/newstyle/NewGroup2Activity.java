            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if("END".equals(eq.getEquipmentDesc())){
                    ...
                    finish();
                        ...
                        || NameSolve.CO_ALARM.equals(NameSolve.getEqType(eq.getEquipmentDesc()))
                        || NameSolve.GAS_ALARM.equals(NameSolve.getEqType(eq.getEquipmentDesc()))
                        || NameSolve.THERMAL_ALARM.equals(NameSolve.getEqType(eq.getEquipmentDesc()))
                        || NameSolve.WT_ALARM.equals(NameSolve.getEqType(eq.getEquipmentDesc()))){
        ...
        output.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if("END".equals(eq.getEquipmentDesc())){
                    ...
                    startActivity(addItem);
                    finish();
                }else if(NameSolve.OUTDOOR_SIREN.equals(NameSolve.getEqType(eq.getEquipmentDesc()))){
                    Intent addItem = new Intent(NewGroup2Activity.this, OutDoorSirenNewActivity.class);
                    ModelConditionPojo mcp = ModelConditionPojo.getInstance();
                    mcp.condition = "output";
                    mcp.position = position;
                    mcp.input = inputShow;
...
//        }
    ...
    private void verfication() {
        ...
        if(TextUtils.isEmpty(getTopBarView().getEditTitleText())){
            ...
            if(getTopBarView().getEditTitleText().getBytes(encode).length>15){
    ...
    private void showTipMask()
    {
        mHightLight = new HighLight(this)//
               // 如果是Activity上增加引导层，不需要设置anchor
                .addHighLight(input.getChildAt(input.getChildCount()-1), R.layout.highlight_info_up, new HighLight.OnPosCallback()
                {
                    /**
                     * @param rightMargin
                     *            高亮view在anchor中的右边距
    ...
    private void showTipMask2()
    {
        mHightLight2 = new HighLight(this)//
                // 如果是Activity上增加引导层，不需要设置anchor
                .addHighLight(output.getChildAt(output.getChildCount()-1), R.layout.highlight_info_down, new HighLight.OnPosCallback()
                {
                    /**
                     * @param rightMargin
                     *            高亮view在anchor中的右边距
    ...
    private void showTipMask3()
    {
        ...
        mHightLight3 = new HighLight(this)//
                // 如果是Activity上增加引导层，不需要设置anchor
                .addHighLight(getTopBarView().getEditTitle(), R.layout.highlight_info_name, new HighLight.OnPosCallback()
                {
                    /**
                     * @param rightMargin
                     *            高亮view在anchor中的右边距
